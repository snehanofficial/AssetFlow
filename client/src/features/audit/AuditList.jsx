...
export const AuditList = () => {
  ...
  if (selectedCycleId) {
      ...
      return (
        <div className="space-y-6 animate-pulse">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {[...Array(4)].map((_, idx) => (
              <div key={idx} className="card-elevation h-24 bg-surface/50 rounded-lg"></div>
            ))}
          </div>
    ...
    if (!cycleDetails) {
      return (
        <div className="card-elevation p-8 text-center text-text-secondary text-sm space-y-3">
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-border/40 pb-4">
          <div className="space-y-2">
            <button
              onClick={() => setSelectedCycleId(null)}
              className="text-text-muted hover:text-text-primary text-xs font-semibold flex items-center gap-1.5 cursor-pointer bg-surface border border-border px-3 py-1 rounded"
            >
            ...
            <div className="space-y-1">
              ...
              <div className="flex flex-wrap items-center gap-3 text-xxs text-text-secondary">
                ...
                <span className="flex items-center gap-1">
                  {isCycleOpen ? (
                    <>
                      <LockOpen className="w-3 h-3 text-primary" /> Active Sheet
                    </>
                  ) : (
                    <>
                      <Lock className="w-3 h-3 text-text-muted" /> Locked & Closed
                    </>
                  )}
                </span>
              </div>
          ...
          {isAdmin && isCycleOpen && (
            <button
              onClick={() => {
                ...
                if (confirm('Are you sure you want to close and lock this audit cycle? This action is irreversible.')) {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="card-elevation p-5 bg-surface flex flex-col justify-between">
          ...
          <div className="card-elevation p-5 bg-surface/50 border-success/20">
            <span className="text-success text-xxs font-semibold uppercase tracking-wider flex items-center gap-1">
              <ShieldCheck className="w-3.5 h-3.5" /> Verified Ok
            </span>
            <span className="text-3xl font-bold font-display text-text-primary block mt-3">
              {stats.verified}
            </span>
            <span className="text-xxs text-text-muted mt-2 block">Confirmed on-site</span>
          </div>

          <div className="card-elevation p-5 bg-surface/50 border-destructive/20">
            <span className="text-destructive text-xxs font-semibold uppercase tracking-wider flex items-center gap-1">
              <AlertTriangle className="w-3.5 h-3.5" /> Missing (Discrepancy)
            </span>
            <span className="text-3xl font-bold font-display text-text-primary block mt-3">
              {stats.missing}
            </span>
            <span className="text-xxs text-text-muted mt-2 block">Will cascade to LOST status</span>
          </div>

          <div className="card-elevation p-5 bg-surface/50 border-warning/20">
            <span className="text-warning text-xxs font-semibold uppercase tracking-wider flex items-center gap-1">
              <AlertTriangle className="w-3.5 h-3.5" /> Damaged (Discrepancy)
            </span>
            <span className="text-3xl font-bold font-display text-text-primary block mt-3">
              {stats.damaged}
            </span>
            <span className="text-xxs text-text-muted mt-2 block">Will trigger repair workflow</span>
          </div>
        </div>

        {/* Auditor List banner */}
        <div className="bg-surface border border-border p-4 rounded-lg flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2">
            <User className="w-4 h-4 text-text-secondary" />
            <span className="text-text-secondary font-semibold">Assigned Auditors:</span>
            <span className="text-text-primary">
              {cycleDetails.auditors.map((a) => a.name).join(', ')}
            </span>
          </div>
          {!isAuditor && isCycleOpen && (
            <div className="text-xxs text-warning font-semibold">
        ...
        <div className="card-elevation p-6 space-y-4">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <h3 className="font-semibold text-sm text-text-primary">Auditor Verification Sheet</h3>
            ...
            <div className="relative w-full sm:w-64">
              ...
              <input
                ...
                className="w-full bg-background border border-border rounded-md pl-9 pr-4 py-1.5 text-xs text-text-primary focus:outline-none focus:border-primary"
              />
            </div>
          </div>

          <div className="overflow-x-auto w-full border border-border rounded-lg bg-background/50">
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="border-b border-border bg-surface text-text-secondary font-semibold uppercase tracking-wider text-xxs">
              ...
              <tbody className="divide-y divide-border/60">
                {filteredItems.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-surface-hover/50 text-text-secondary">
                      <td className="px-4 py-4 whitespace-nowrap font-mono font-bold text-text-primary text-[11px]">
                        {item.asset.assetTag}
                      </td>
                      <td className="px-4 py-4">
                        <p className="font-medium text-text-primary leading-snug">{item.asset.name}</p>
                        <p className="text-[10px] text-text-muted mt-0.5">Location: {item.asset.location}</p>
                      </td>
                      <td className="px-4 py-4">
                        <span className="text-[10px] text-text-muted bg-surface px-2 py-0.5 rounded border border-border/50">
                          {item.asset.status}
                        </span>
                      </td>
                      <td className="px-4 py-4">
                        {canAuditItem ? (
                          <select
                            ...
                            className="bg-background border border-border rounded text-xs px-2.5 py-1 focus:outline-none focus:border-primary text-text-primary font-medium"
                          >
                          ...
                          <div>
                            ...
                            <span className="block text-[9px] text-text-muted mt-1 font-normal italic">
                              {verifiedByStr}
                            </span>
                          </div>
                        )}
                      ...
                      <td className="px-4 py-4 max-w-xs">
                        {canAuditItem ? (
                          <input
                            ...
                            className="w-full bg-background border border-border/80 rounded px-2 py-1 text-xs text-text-primary focus:outline-none focus:border-primary"
                          />
                        ) : (
                          <p className="italic leading-relaxed text-xxs">
                            {item.notes || <span className="text-text-muted/60">No notes recorded</span>}
                          </p>
                        )}
                      </td>
                      {canAuditItem && (
                        <td className="px-4 py-4 text-right whitespace-nowrap">
                          <button
                            ...
                            className="text-primary hover:text-primary-hover font-semibold text-xxs py-1 px-2 border border-primary/20 hover:border-primary rounded bg-primary/5 transition-all cursor-pointer"
                          >
                            Save
                          </button>
                        </td>
                ...
                {filteredItems.length === 0 && (
                  <tr>
                    <td colSpan={6} className="text-center py-10 text-text-muted text-xs italic">
      ...
      <div className="card-elevation p-6 space-y-4">
        <div className="flex justify-between items-center border-b border-border pb-3">
          <span className="text-sm font-semibold text-text-primary flex items-center gap-1.5">
            <ClipboardList className="w-4.5 h-4.5 text-text-secondary" />
            Verification Campaigns
          </span>
          <button
            onClick={() => queryClient.invalidateQueries({ queryKey: ['audits'] })}
            className="p-1.5 text-text-muted hover:text-text-primary rounded border border-border/80 bg-surface/50 cursor-pointer"
          >
              ...
              <tbody className="divide-y divide-border/60">
                {cycles.map((item) => (
                  <tr key={item.id} className="hover:bg-surface-hover/50 text-text-secondary">
                    ...
                    <td className="px-4 py-3.5 font-mono text-[10px] text-text-primary">
                    ...
                    <td className="px-4 py-3.5 text-right whitespace-nowrap">
                      <button
                        ...
                        className="text-primary hover:underline font-semibold text-xxs inline-flex items-center gap-1 border border-border hover:border-primary/20 bg-surface px-2.5 py-1 rounded transition-all cursor-pointer"
                      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 animate-fade-in">
          <div className="bg-surface border border-border w-full max-w-md rounded-lg p-6 space-y-6 shadow-2xl relative animate-slide-up">
            ...
            <form onSubmit={handleSubmitCreate(handleCreateSubmit)} className="space-y-4">
              {/* Cycle Title */}
              <div className="space-y-1">
                <label className="text-text-secondary text-xs font-semibold" htmlFor="audit-title-input">
                  Campaign Title
                </label>
                <input
                  ...
                  className={`w-full bg-background border rounded-md px-3 py-2 text-sm text-text-primary focus:outline focus:border-primary ${
                    createErrors.title ? 'border-destructive' : 'border-border'
                  }`}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-text-secondary text-xs font-semibold" htmlFor="audit-scope-type">
                    Scope Type
                  </label>
                  <select
                    id="audit-scope-type"
                ...
                <div className="space-y-1">
                  <label className="text-text-secondary text-xs font-semibold" htmlFor="audit-scope-value">
                    Scope Target
                  </label>
                  {scopeType === 'location' ? (
                    <input
              ...
              {/* Due Date */}
              <div className="space-y-1">
                <label className="text-text-secondary text-xs font-semibold" htmlFor="audit-due-date">
                  Verify Due Date
                </label>
                <input
                  id="audit-due-date"
                  type="date"
                  className={`w-full bg-background border rounded-md px-3 py-2 text-sm text-text-primary focus:outline focus:border-primary ${
                    createErrors.endDate ? 'border-destructive' : 'border-border'
                  }`}
              ...
              {/* Assign Auditors */}
              <div className="space-y-2">
                <label className="text-text-secondary text-xs font-semibold block">
                  Assign Auditors
                </label>
                <div className="border border-border rounded-md p-3 max-h-32 overflow-y-auto space-y-2 bg-background/50">
                  {employees.map((emp) => (
                    <label
                      key={emp.id}
